(function (global, factory) {
	typeof exports === 'object' && typeof module !== 'undefined' ? factory(exports, require('tty'), require('buffer'), require('zlib'), require('events'), require('stream'), require('util'), require('querystring'), require('net'), require('http'), require('crypto'), require('path'), require('url'), require('fs')) :
	typeof define === 'function' && define.amd ? define(['exports', 'tty', 'buffer', 'zlib', 'events', 'stream', 'util', 'querystring', 'net', 'http', 'crypto', 'path', 'url', 'fs'], factory) :
	(factory((global.statusRouter = {}),global.tty,global.buffer,global.zlib,global.events,global.stream,global.util,global.querystring,global.net,global.http,global.crypto,global.path,global.url,global.fs));
}(this, (function (exports,tty,buffer,zlib,events,stream,util,querystring,net,http,crypto,path,url,fs) { 'use strict';
	...
	function SBCSCodec(codecOptions, iconv) {
	    ...
	    this.decodeBuf = new Buffer$6.from(codecOptions.chars, 'ucs2');
	    
	    // Encoding buffer.
	    var encodeBuf = new Buffer$6.alloc(65536, iconv.defaultCharSingleByte.charCodeAt(0));
	...
	var extendNode = function (iconv) {
	    ...
	    iconv.supportsNodeEncodingsExtension = !(Buffer$9.from || new Buffer$9(0) instanceof Uint8Array);
	...
	function InternalDecoder$1(options, codec) {
	    StringDecoder$2.call(this, codec.enc);
	}

	InternalDecoder$1.prototype = StringDecoder$2.prototype;
	...
	Utf16BEEncoder$1.prototype.write = function(str) {
	    var buf = Buffer$b.from(str, 'ucs2');
	    for (var i = 0; i < buf.length; i += 2) {
	        var tmp = buf[i]; buf[i] = buf[i+1]; buf[i+1] = tmp;
	    }
	    return buf;
	...
	Utf16BEDecoder$1.prototype.write = function(buf) {
	    ...
	    var buf2 = Buffer$b.alloc(buf.length + 1),
	        i = 0, j = 0;
	...
	Utf16Decoder$1.prototype.write = function(buf) {
	    if (!this.decoder) {
	        ...
	        // We have enough bytes -> detect endianness.
	        var buf = Buffer$b.concat(this.initialBytes),
	...
	Utf7Encoder$1.prototype.write = function(str) {
	    // Naive implementation.
	    // Non-direct chars are encoded as "+<base64>-"; single "+" char is encoded as "+-".
	    return Buffer$c.from(str.replace(nonDirectChars$1, function(chunk) {
	        return "+" + (chunk === '+' ? '' : 
	            this.iconv.encode(chunk, 'utf16-be').toString('base64').replace(/=+$/, '')) 
	            + "-";
	    }.bind(this)));
	        ...
	        if (!inBase64) { // We're in direct mode.
	            ...
	            if (!base64Chars$1[buf[i]]) { // Base64 ended.
	                if (i == lastI && buf[i] == minusChar$1) {// "+-" -> "+"
	                    ...
	                    var b64str = base64Accum + buf.slice(lastI, i).toString();
	                    res += this.iconv.decode(Buffer$c.from(b64str, 'base64'), "utf16-be");
	                }
	...
	Utf7Decoder$1.prototype.end = function() {
	    var res = "";
	    if (this.inBase64 && this.base64Accum.length > 0)
	        res = this.iconv.decode(Buffer$c.from(this.base64Accum, 'base64'), "utf16-be");
	...
	function Utf7IMAPEncoder$1(options, codec) {
	    this.iconv = codec.iconv;
	    this.inBase64 = false;
	    this.base64Accum = Buffer$c.alloc(6);
	    this.base64AccumIdx = 0;
	}
	...
	Utf7IMAPEncoder$1.prototype.write = function(str) {
	    var inBase64 = this.inBase64,
	        base64Accum = this.base64Accum,
	        base64AccumIdx = this.base64AccumIdx,
	        buf = Buffer$c.alloc(str.length*5 + 10), bufIdx = 0;
	...
	Utf7IMAPEncoder$1.prototype.end = function() {
	    var buf = Buffer$c.alloc(10), bufIdx = 0;
	    if (this.inBase64) {
	        if (this.base64AccumIdx > 0) {
	            bufIdx += buf.write(this.base64Accum.slice(0, this.base64AccumIdx).toString('base64').replace(/\//g, ',').replace(/=+$/, ''), bufIdx);
	            this.base64AccumIdx = 0;
	...
	function SBCSCodec$1(codecOptions, iconv) {
	    ...
	    this.decodeBuf = new Buffer$d.from(codecOptions.chars, 'ucs2');
	    
	    // Encoding buffer.
	    var encodeBuf = new Buffer$d.alloc(65536, iconv.defaultCharSingleByte.charCodeAt(0));
	...
	IconvLiteEncoderStream$1.prototype.collect = function(cb) {
	    ...
	    this.on('data', function(chunk) { chunks.push(chunk); });
	    this.on('end', function() {
	        cb(null, Buffer$f.concat(chunks));
	    });
	    return this;
	};
	...
	var extendNode$1 = function (iconv) {
	    ...
	    // Uint8Array and we cannot patch key functions since then.
	    // Note: this does use older Buffer API on a purpose
	    iconv.supportsNodeEncodingsExtension = !(Buffer$g.from || new Buffer$g(0) instanceof Uint8Array);
	    ...
	    iconv.extendNodeEncodings = function extendNodeEncodings() {
	        ...
	        Buffer$g.isNativeEncoding = function(enc) {
	            return enc && nodeNativeEncodings[enc.toLowerCase()];
	        };
	        ...
	        SlowBuffer.prototype.toString = function(encoding, start, end) {
	            ...
	            // Use native conversion when possible
	            if (Buffer$g.isNativeEncoding(encoding))
	                return original.SlowBufferToString.call(this, encoding, start, end);
	        ...
	        original.BufferIsEncoding = Buffer$g.isEncoding;
	        Buffer$g.isEncoding = function(encoding) {
	            return Buffer$g.isNativeEncoding(encoding) || iconv.encodingExists(encoding);
	        };

	        original.BufferByteLength = Buffer$g.byteLength;
	        Buffer$g.byteLength = SlowBuffer.byteLength = function(str, encoding) {
	            encoding = String(encoding || 'utf8').toLowerCase();
	        ...
	        original.BufferToString = Buffer$g.prototype.toString;
	        Buffer$g.prototype.toString = function(encoding, start, end) {
	            encoding = String(encoding || 'utf8').toLowerCase();
	        ...
	        original.BufferWrite = Buffer$g.prototype.write;
	        Buffer$g.prototype.write = function(string, offset, length, encoding) {
	            var _offset = offset, _length = length, _encoding = encoding;
	            // Support both (string, offset, length, encoding)
	            // and the legacy (string, encoding, offset, length)
	            if (isFinite(offset)) {
	    ...
	    iconv.undoExtendNodeEncodings = function undoExtendNodeEncodings() {
	        ...
	        delete Buffer$g.isNativeEncoding;
	        ...
	        Buffer$g.isEncoding = original.BufferIsEncoding;
	        Buffer$g.byteLength = original.BufferByteLength;
	        Buffer$g.prototype.toString = original.BufferToString;
	        Buffer$g.prototype.write = original.BufferWrite;
	...
	function read (req, res, next, parse, debug, options) {
	  ...
	  if (opts.encoding === null && encoding !== null && !lib$1.encodingExists(encoding)) {
	...
	function tryStat(path$$1) {
	  ...
	  try {
	    return fs__default.statSync(path$$1);
	  } catch (e) {
	    return undefined;
	  }
	}
	...
	var require$$0$3 = getCjsExportFromNamespace(types$1);
	...
	SendStream.prototype.sendFile = function sendFile (path$$1) {
	  ...
	  fs__default.stat(path$$1, function onstat (err, stat) {
	...
	SendStream.prototype.stream = function stream$$1 (path$$1, options) {
	  ...
	  // pipe
	  var stream$$1 = fs__default.createReadStream(path$$1, options);
	...
	res.send = function send(body) {
	  ...
	  switch (typeof chunk) {
	    ...
	    case 'object':
	      if (chunk === null) {
	        chunk = '';
	      } else if (Buffer$h.isBuffer(chunk)) {
	        if (!this.get('Content-Type')) {
	          this.type('bin');
	        }
	      } else {
	  ...
	  var len;
	  if (chunk !== undefined) {
	    if (Buffer$h.isBuffer(chunk)) {
	      // get length of Buffer
	      len = chunk.length;
	    } else if (!generateETag && chunk.length < 1000) {
	      // just calculate length when no ETag + small chunk
	      len = Buffer$h.byteLength(chunk, encoding);
	    } else {
	...
	res.redirect = function redirect(url$$1) {
	  ...
	  this.set('Content-Length', Buffer$h.byteLength(body));
	...
	router$1.get("/health", health);
	router$1.get("/version", version);
